...
func newMiddleMan(config *rest.Config, name string) (*middleMan, error) {
	m := &middleMan{}
	...
	m.Logger = l.WithName(fmt.Sprintf("middleman-on-%s", name))
	ms.Logger = l.WithName(fmt.Sprintf("middleman-status-on-%s", name))
