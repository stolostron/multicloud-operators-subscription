...
var (
	DefaultRepos = map[string]map[string]string{
		...
		"placementrule": map[string]string{
		...
		"managedcluster": map[string]string{
...
func entry() {
	...
	tests := []string{"deployable", "channel", "ansiblejob"}
	if err := UpdateCRDs(givenPath, tests, logger); err != nil {
		fmt.Println(err)
		logger.Error(err, "failed to update CRDs")
	}
}
...
func download(repos []string, out string, logger logr.Logger) error {
	for _, repo := range repos {
		...
		if !ok {
			return errors.New(fmt.Sprintf("the %v is not in the default list", repo))
		}
