...
// Note: unlike the main branch version of this fix, the
// TestObjectBucketDoSubscriptionRespectsClusterAdmin test (which exercises
// doSubscription() end to end via a fake S3 server) is intentionally omitted
// here because pkg/utils.SetupFakeS3Server does not exist on this release
// branch (it was introduced by a later, unrelated PR). The security fix
// itself is unaffected; TestObjectBucketRespectsClusterAdminAnnotation above
// already covers the same ProcessSubResources behavior this test would
// exercise, just without going through the real S3-backed subscription path.
