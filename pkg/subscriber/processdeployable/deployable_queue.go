...
type SyncSource interface {
	...
	AddTemplates(string, types.NamespacedName, []kubesynchronizer.DplUnit, map[string]map[string]string) error
}
...
func Units(sub *subv1.Subscription, synchronizer SyncSource,
	...
	pkgMap map[string]bool, dplUnits []kubesynchronizer.DplUnit, allowlist map[string]map[string]string) error {
	if err := synchronizer.AddTemplates(syncsource, hostkey, dplUnits, allowlist); err != nil {
