...
/*func TestHelmReleaseOverride(t *testing.T) {
	g := gomega.NewGomegaWithT(t)
...
data:
  path: test/github/helmcharts`
	...
	subYaml := `apiVersion: app.ibm.com/v1alpha1
kind: Subscription
metadata:
  name: githubtest
spec:
  channel: default/githubtest
  name: chart1
  placement:
    local: true
  packageFilter:
  ...
  - packageName: chart1
	...
	testSubscrp := &appv1alpha1.Subscription{}
	err = yaml.Unmarshal([]byte(subYaml), &testSubscrp)
	g.Expect(err).NotTo(gomega.HaveOccurred())

	subitem := &SubscriberItem{}
	subitem.Subscription = testSubscrp
	subitem.Channel = githubchn
	subitem.synchronizer = defaultSubscriber.synchronizer
	...
	overrides := subitem.getOverrides("chart1")
